<?php
    ...
    abstract class Model {
        ...
        final public function updateByFieldName(string $fieldName, $newValue, $oldValue) {
            if (!$this->isFieldValueValid($fieldName, $newValue)) {
                throw new \Exception('Invalid field name or value: ' . $fieldName);
            }
            $tableName = $this->getTableName();
            $sql = 'UPDATE ' . $tableName . ' SET ' . $fieldName . ' = ? WHERE ' . $fieldName . ' = ?;';
            $prep = $this->dbc->getConnection()->prepare($sql);
            return $prep->execute([ $newValue, $oldValue ]);            
        }
